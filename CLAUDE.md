...
│   │   ├── ScheduledTaskScheduler.swift  # GRDB ValueObservation + one-shot Timer per due slot, missed-run reconciliation
...
│   │   └── ScheduledTaskPowerGuard.swift # IOPMAssertion holder — prevents idle sleep / screen saver while any scheduled session is alive
│   ├── Settings/                   # Settings
...
- **Scheduler**: `ScheduledTaskScheduler` is event-driven, **no polling**. A GRDB `ValueObservation` watches enabled tasks ordered by `nextRunAt`; on every DB change it cancels and re-schedules a single one-shot `Timer` for the earliest future deadline. When the timer fires, any task whose `nextRunAt` has passed is handed off to the executor; the executor's writes (`markRunStarted`, etc.) round-trip through the observation to advance the schedule. On launch it rolls `nextRunAt` forward for any task whose due time was missed while the app was closed — missed runs are **never** fired (decision: skip-missed).
...
- **Window delegate gotcha**: `WindowDelegate` is attached in two places — `AppDelegate.applicationDidFinishLaunching` (existing windows on launch) and `AppDelegate.handleWindowBecameKey` (lazy assignment on first key change). Background-spawned scheduled windows never become key from `orderFront(nil)`, so neither path runs. Without the delegate, clicking the red X just hides the window (`isReleasedWhenClosed = false`) and `windowShouldClose` never fires — the session, claude process, and `WindowSessionRegistry` entries leak; selecting from the sidebar reopens the hidden window. `ScheduledTaskExecutor.openBackgroundWindow` therefore attaches `AppDelegate.windowDelegate` (now `internal`) directly to the new window right after creating it.
...
- **Idle sleep / screen saver**: `ScheduledTaskPowerGuard` (`Tenvy/Features/Scheduled/`) holds `IOPMAssertion`s (`PreventUserIdleSystemSleep` + `PreventUserIdleDisplaySleep`) while at least one scheduled-task session is alive. Registered by `ScheduledTaskExecutor.execute` after `openBackgroundWindow`; unregistered (1-second debounce) from `AppModel.deactivateSession`. Tracking is keyed by `tenvySessionId` so it survives the temp-UUID → Claude session-ID swap in `syncSessionFromHookEvent` — the debounce is what keeps the assertion stable across that transient deactivate-then-activate. IOKit auto-releases on process exit if we crash; explicit release happens when the active set drains.
...
- **Edit / no Run Now**: tasks are editable via the pencil button in `ScheduledTaskDetailView`'s header — opens `CreateScheduledTaskView(editing: task)`, which pre-fills the form from the record and saves through `ScheduledTaskStore.update`. The update preserves `id`, `createdAt`, `enabled`, and every `lastRun*` field; `nextRunAt` is re-anchored from "now" using the (possibly new) frequency, matching the existing re-enable semantics. There is still no manual "Run now" trigger — the schedule is the only execution source. Disabling and re-enabling uses **a fresh anchor from re-enable time**, not the original schedule.
